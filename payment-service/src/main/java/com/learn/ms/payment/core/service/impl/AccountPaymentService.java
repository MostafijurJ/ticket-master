...
public class AccountPaymentService extends BaseService implements PaymentService {
    private final TransactionHistoryRepository transactionHistoryRepository;
    private final TransactionHistoryMapper mapper;

    @Override
    public PaymentResponse processPayment(PaymentRequest request) {
        logger.trace("Processing payment for account: {}", request);

        // TODO external account payment related logic will be implemented here
        TransactionHistory transactionHistory = mapper.mapToEntity(request);

        transactionHistoryRepository.save(transactionHistory);
        return PaymentResponse.builder()
                .transactionStatus(TransactionStatus.SUCCESS)
                .paymentSourceType(PaymentSourceType.ACCOUNT)
